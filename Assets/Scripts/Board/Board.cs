...
public class Board
{
    ...
    internal void Fill()
    {
        ...
        for (int i = 0; i < 3; i++)
        {
            itemTypesPool.Add(NormalItem.eNormalType.TYPE_ONE);
            itemTypesPool.Add(NormalItem.eNormalType.TYPE_TWO);
            itemTypesPool.Add(NormalItem.eNormalType.TYPE_THREE);
            itemTypesPool.Add(NormalItem.eNormalType.TYPE_FOUR);
            itemTypesPool.Add(NormalItem.eNormalType.TYPE_FIVE);
            itemTypesPool.Add(NormalItem.eNormalType.TYPE_SIX);
            itemTypesPool.Add(NormalItem.eNormalType.TYPE_SEVEN);
        }

        for (int shuffle = 0; shuffle < 3; shuffle++)
        {
            for (int i = 0; i < itemTypesPool.Count; i++)
            {
                NormalItem.eNormalType temp = itemTypesPool[i];
                int randomIndex = UnityEngine.Random.Range(0, itemTypesPool.Count);
                itemTypesPool[i] = itemTypesPool[randomIndex];
                itemTypesPool[randomIndex] = temp;
            }
        }
        ...
        for (int x = 0; x < boardSizeX; x++)
        {
            for (int y = 0; y < boardSizeY; y++)
            {
                ...
                NormalItem.eNormalType selectedType = itemTypesPool[itemIndex];

                List<NormalItem.eNormalType> forbiddenTypes = new List<NormalItem.eNormalType>();
                ...
                if (cell.NeighbourBottom != null)
                {
                    ...
                    if (nitem != null)
                    {
                        forbiddenTypes.Add(nitem.ItemType);
                        if (cell.NeighbourBottom.NeighbourBottom != null)
                        {
                            NormalItem nitem2 = cell.NeighbourBottom.NeighbourBottom.Item as NormalItem;
                            if (nitem2 != null && nitem2.ItemType == nitem.ItemType)
                            {
                                forbiddenTypes.Add(nitem.ItemType);
                            }
                        }
                ...
                if (cell.NeighbourLeft != null)
                {
                    ...
                    {
                        forbiddenTypes.Add(nitem.ItemType);
                        if (cell.NeighbourLeft.NeighbourLeft != null)
                        {
                            NormalItem nitem2 = cell.NeighbourLeft.NeighbourLeft.Item as NormalItem;
                            if (nitem2 != null && nitem2.ItemType == nitem.ItemType)
                            {
                                forbiddenTypes.Add(nitem.ItemType);
                            }
                ...
                if (forbiddenTypes.Contains(selectedType))
                {
                    bool found = false;
                    for (int j = itemIndex + 1; j < itemTypesPool.Count; j++)
                    {
                        if (!forbiddenTypes.Contains(itemTypesPool[j]))
                        {
                            NormalItem.eNormalType temp = itemTypesPool[itemIndex];
                            itemTypesPool[itemIndex] = itemTypesPool[j];
                            itemTypesPool[j] = temp;
                            selectedType = itemTypesPool[itemIndex];
                            found = true;
                            break;
                        }
                    }
        ...
        for (int x = 0; x < boardSizeX; x++)
        {
            for (int y = 0; y < boardSizeY; y++)
            {
                ...
                if (!cell.IsEmpty)
                {
                    ...
                    {
                        isExcluded = excludedCells.Contains(cell);
                    }
